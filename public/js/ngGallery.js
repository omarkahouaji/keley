(function () {
    ...
    function ngGallery($document, $timeout, $q, $templateCache) {
        ...
        $templateCache.put(template_url,
            '<div class="{{ baseClass }}">' +
            '  <div ng-repeat="i in images"  ng-if="$index< 4">' +
            '<div id="{{i.id}}" class="col-xs-6 col-md-3 center"><div class="events_img"  >'+
            '    <img ng-src="{{ i.thumb | trusted }}" class=" img-responsive" ng-click="openGallery($index)"  alt="Image {{ i.id}} " />' +
            ' <div alt="Image {{ $index}}" ng-src="{{ i.thumb | trusted }}" ng-click="openGallery($index)" class="overlay" ng-class="($index==3 && images.length>4) ? \'overlay-pass\' : \'\' "><span class="imagesCounter">+{{images.length-4}}</span></div></div></div> '+
            '  </div>' +
            '</div>' +
            ...
            '  <a class="close-popup" ng-click="closeGallery()"><i class="icon-close fs-30"></i></a>' +
             '  <a ng-click="delete($event,id,event_id)">Supprimer</a>' +
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                var loadImage = function (i) {
                    ...
                    image.id = scope.images[i].id; 
                    image.event_id = scope.images[i].event_id; 
                    scope.loading = true;
                ...
                scope.delete = function (event,i,event_id) {
                    console.log(event);
                    scope.opened = false;
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            if(json.msg=='success'){   
                                ...
                                console.log('Erreur deleting image')
                            }
                        }
                    });
