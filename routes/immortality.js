...
var upload_path = 'http://immortality-life-api.azurewebsites.net/Immortality-api/application/uploads/';
        ...
        request({
            ...
            function (error, response, body) {
                request({
                    ...
                    function (err, response, activity_body) {
                        if (err) throw err;
                        var json = JSON.parse(body);
            ...
            function (error, response, fdatab) {
                ...
                request({ url: base_url + 'charts/all/' + parseInt(req.params.id), method: 'GET' },
                    function (error, response, body) {
                        ...
                        res.render('chartsFriend', {
                            charts: JSON.stringify(courbes), informations: retour.data[0],
        ...
        function (error, response, body) {
            ...
            if (type != 'undefined' && size != 0) {
                ...
                Jimp.read(src, function (err, image) {
                    if (err) throw err;
                    image.cover(150, 150)            // resize
                        .write('small_' + req.files.file.originalFilename); // save
                });
                Jimp.read(src, function (err, image) {
                    if (err) throw err;
                    image.contain(1024, 768)            // resize
                        .write(req.files.file.originalFilename, function () {
                            request({
            ...
            //end single
                ...
                function upload(i) {
                    if (i < srcPath.length) {
                        Jimp.read(srcPath[i], function (err, image) {
                            if (err) throw err;
                            image.cover(150, 150)
                                .write('small_' + req.files.file[i].originalFilename);
                        });
                        Jimp.read(srcPath[i], function (err, image) {
                            if (err) throw err;
                            image.contain(1024, 768)
                                .quality(60)
                                .write(req.files.file[i].originalFilename, function () {
                                    request({
                                        url: base_url + 'uploads/postImages',
                                        method: 'POST',
                                        formData: {
